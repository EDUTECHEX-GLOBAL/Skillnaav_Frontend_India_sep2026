      ...
      {acceptedOffers.length === 0 ? (
        <div className="bg-white p-8 text-center rounded-xl border border-gray-200 shadow-sm">
          ...
          <p className="text-gray-500 font-medium">
            No active internships found to track attendance.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {acceptedOffers.map((offer) => {
            const iid =
              typeof offer.internshipId === "object" &&
              offer.internshipId !== null
                ? offer.internshipId._id || String(offer.internshipId)
                : String(offer.internshipId);
            const title =
              offer.internshipTitle || offer.position || "Internship";
            return (
    ...
    const fetchSessionStatus = async () => {
      try {
        ...
        const res = await axios.get(
          `/api/attendance/session-status/${internshipId}`,
          {
  ...
  const handleSubmitOtp = async (e) => {
    ...
    try {
      ...
      setSessionStatus((prev) =>
        prev ? { ...prev, alreadyMarked: true } : null,
      );

      setTimeout(() => {
        const fetchAttendance = async () => {
          const res = await axios.get(`/api/attendance/my/${internshipId}`, {
            headers: { Authorization: `Bearer ${userToken}` },
          });
          setData(res.data);
        };
          ...
          {error ? (
            ...
            <div className="mt-4">
              ...
              <div className="flex justify-between items-center text-[10px] font-semibold text-gray-500 mb-3">
                ...
                <span
                  className={`${!data?.eligible ? "text-orange-600" : "text-emerald-600"}`}
                >
            ...
            <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-xl shrink-0">
              <div>
                <h3 className="text-lg font-bold text-gray-800">
                  {job?.internshipTitle || internshipTitle}
                </h3>
                  ...
                  <div className="mb-6 rounded-xl p-5 flex items-center gap-4 shadow-sm bg-gradient-to-r from-emerald-50 to-emerald-100 border border-emerald-200">
                    ...
                    <div className="flex-grow">
                      ...
                      <p className="text-sm mt-0.5 text-emerald-700">
                        Your internship completion certificate is ready to
                        download.
                      </p>
                    </div>
                    <a
                  ...
                  <div className="mb-6 rounded-xl p-5 flex items-center gap-4 shadow-sm bg-gradient-to-r from-amber-50 to-yellow-50 border border-amber-200">
                    ...
                    <div className="flex-grow">
                      ...
                      <p className="text-sm mt-0.5 text-amber-700">
                        You met the {data?.minAttendancePercent}% attendance
                        requirement ({data?.percent}%). Your certificate is
                  ...
                  <div className="mb-6 rounded-xl p-5 flex items-center gap-4 shadow-sm bg-gradient-to-r from-red-50 to-orange-50 border border-red-200">
                    ...
                    <div className="flex-grow">
                      ...
                      <p className="text-sm mt-0.5 text-red-600">
                        Your attendance ({data?.percent || 0}%) did not meet the
                        minimum requirement of {data?.minAttendancePercent || 0}
                        %.
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex justify-between items-center text-sm font-semibold">
                  <span className="text-gray-500">
                    ...
                    <span className="text-gray-800">
                      {data?.minAttendancePercent || 0}%
                    </span>
                  </span>
                  <span
                    className={`px-2.5 py-1 rounded-md text-xs flex items-center ${!data?.eligible ? "bg-orange-100 text-orange-700" : "bg-emerald-100 text-emerald-700"}`}
                  >
                    <FontAwesomeIcon
                      icon={
                        !data?.eligible ? faExclamationCircle : faCheckCircle
                      }
                      className="mr-1.5"
                    />
                    {!data?.eligible ? "At Risk" : "On Track"}
                ...
                <div className="mb-8">
                    ...
                    !sessionStatus.alreadyMarked && (
                      <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-6 shadow-sm relative overflow-hidden group">
                        ...
                        <p className="text-indigo-700 text-sm mb-5 pl-2 font-medium">
                          Your instructor has shared a 4-digit attendance code.
                          Enter it below to mark yourself present:
                        </p>

                        <form
                          onSubmit={handleSubmitOtp}
                          className="flex flex-col sm:flex-row gap-3 items-start sm:items-center pl-2"
                        >
                          <input
                            type="text"
                            maxLength="4"
                            ...
                            placeholder="••••"
                            className="!mt-0 px-4 py-2 text-2xl tracking-[0.5em] font-mono text-center w-36 rounded-lg border-2 border-indigo-200 focus:outline-none focus:border-indigo-500 focus:ring-0 bg-white shadow-inner"
                          />
                          <button
                            type="submit"
                        ...
                        {otpMessage && (
                          <div
                            className={`mt-4 pl-2 text-sm font-bold ${otpMessage.type === "error" ? "text-red-600" : "text-emerald-600"}`}
                          >
                            {otpMessage.text}
                          </div>
                        )}
                  ...
                  {sessionStatus.alreadyMarked && (
                    <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-5 flex items-center text-emerald-800 shadow-sm">
                      ...
                      <div>
                        <span className="font-bold block text-lg text-emerald-900">
                          Present Today
                        </span>
                  ...
                  <div className="border border-gray-200 rounded-xl overflow-hidden">
                    <table className="w-full text-left text-sm">
                      <thead className="bg-gray-100 text-gray-600 border-b border-gray-200">
                        <tr>
                          <th className="px-5 py-3 font-bold whitespace-nowrap text-xs uppercase">
                            Date
                          </th>
                          <th className="px-5 py-3 font-bold whitespace-nowrap text-xs uppercase">
                            Type
                          </th>
                          <th className="px-5 py-3 font-bold whitespace-nowrap text-xs uppercase text-right">
                            Status
                          </th>
                        </tr>
                      </thead>
